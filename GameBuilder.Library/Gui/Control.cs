...
namespace GameBuilder.Library.Gui
{
    public class Control
    {
        ...
        public virtual void Render()
        {
            ...
            Graphics.GraphicsManager.TranslateWorld(new Vector3(_body.X, _body.Y, -_zDepth));

            Graphics.GraphicsManager.PushStencilDepth(StencilOp.Incr);
            ...
            Graphics.GraphicsManager.FillRoundedRect(0, 0, 0, _body.Width, _body.Height, _borderRadius, _backgroundColor);

            //Graphics.PushStencilDepth(StencilOp.Keep);
            Graphics.GraphicsManager.SetStencilOp(StencilOp.Keep);

            if (_parent == null)
            {
                Graphics.GraphicsManager.PushScreenClip(_content);
            }
            else
            {
            ...
            Graphics.GraphicsManager.PushWorldMatrix();
            Graphics.GraphicsManager.TranslateWorld(new Vector3(GetMargin(), GetMargin(), 0));

            this.RenderContent();

            Graphics.GraphicsManager.PopStencilDepth();

            Graphics.GraphicsManager.PopWorldMatrix();
            Graphics.GraphicsManager.PopScreenClip();

            Graphics.GraphicsManager.DrawRect(0, 0, 0, _body.Width, _body.Height, 1, _borderColor);
            float padding = 0.25f * _borderThickness;//offsetting the rect drawing so its completely inside the clipped background
            Graphics.GraphicsManager.DrawRoundedRect(padding, padding, 0, _body.Width - (padding * 2) , _body.Height - (padding * 2), _borderRadius, _borderThickness, _borderColor);
