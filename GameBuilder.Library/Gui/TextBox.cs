    ...
    public class TextBox : ScrollPanel, KeyListener
    {
        ...
        public override void RenderContent()
        {
            ...
            for (int i = 0; i < _lines.Count; i++)
            {
                ...
                Graphics.GraphicsManager.DrawText(text, 0, lineHeight - font.GetHeight(text) + (i * lineHeight), 0, Color4.Black);
            }
            if (_active)
            {
                int cursorX = font.GetWidth(_lines[_cursorRow].Substring(0, _cursorColumn)) + 1;
                Graphics.GraphicsManager.FillRect(cursorX, (_cursorRow * lineHeight) + 1, 0, 2, lineHeight, Color4.Black);
            }
        }
