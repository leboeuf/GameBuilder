...
namespace GameBuilder.Library.Gui
{
    public class ListBox : ScrollPanel
    {
        ...
        public override void RenderContent()
        {
            ...
            if (_index >= 0)
            {
                Graphics.GraphicsManager.FillRect(0, _index * 32, 0, _content.Width, 32, _selectionColor);
            }
            for (int i = 0; i < _items.Count; i++)
            {
                int x = (_content.Width / 2) - (Graphics.GraphicsManager.GetFont().GetWidth(_items[i]) / 2);
                int y = 16 - (Graphics.GraphicsManager.GetFont().GetHeight(_items[i]) / 2) + (i * 32);
                Graphics.GraphicsManager.DrawText(_items[i], x, y, 0, Color4.Black);
            }
        }
    }
}
