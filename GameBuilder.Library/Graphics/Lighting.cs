...
namespace GameBuilder.Library.Graphics
{
    public class Lighting
    {
        ...
        {
            int uLightingEnabledLocation = GraphicsManager.GetShader().GetUniformLocation("uLightingEnabled");
        ...
        public static void InitLights()
        {
            for (int i = 0; i < _numLights; i++)
            {
                ...
                int uLightOnLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].On");
                GL.Uniform1(uLightOnLocation, _lights[i].On ? 1 : 0);

                int uLightTypeLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].Type");
                GL.Uniform1(uLightTypeLocation, (int)_lights[i].Type);

                int uLightSpotCutOffLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].SpotCutOff");
                GL.Uniform1(uLightSpotCutOffLocation, _lights[i].CuttOff);

                int uAmbientLightLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].AmbientLight");
                GL.Uniform3(uAmbientLightLocation, _lights[i].AmbientColour);

                int uDiffuseLightLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].DiffuseLight");
                GL.Uniform3(uDiffuseLightLocation, _lights[i].DiffuseColour);

                int uSpecularLightLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].SpecularLight");
                GL.Uniform3(uSpecularLightLocation, _lights[i].SpecularColour);

                int uConstantAttenuationLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].ConstantAttenuation");
                GL.Uniform1(uConstantAttenuationLocation, _lights[i].ConstantAttenuation);

                int uLinearAttenuationLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].LinearAttenuation");
                GL.Uniform1(uLinearAttenuationLocation, _lights[i].LinearAttenuation);

                int uQuadraticAttenuationLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].QuadraticAttenuation");
                GL.Uniform1(uQuadraticAttenuationLocation, _lights[i].QuadraticAttenuation);

                int uIntensityLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].Intensity");
        ...
        public static void SetLightPosition(int light, Vector3 position)
        {
            ...
            GraphicsManager.TranslateWorld(Vector3.Zero);
        }
        ...
        public static void SetLightSwitch(int light, bool state)
        {
            ...
            int uLightOnLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + light + "].On");
            GL.Uniform1(uLightOnLocation, _lights[light].On ? 1 : 0);
        }

        public static void ApplyLightMatrix(Matrix4 world)
        {
            int uEyePosition = GraphicsManager.GetShader().GetUniformLocation("uEyePosition");
            ...
            for (int i = 0; i < _numLights; i++)
            {
                ...
                int uLightPositionLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].Position");
                ...
                int uLightDirectionLocation = GraphicsManager.GetShader().GetUniformLocation("uLight[" + i + "].Direction");
